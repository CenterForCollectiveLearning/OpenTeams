...
//        var CHARGE_DEFAULT = -300;
        var CHARGE_DEFAULT = - Math.max(document.documentElement.clientHeight, window.innerHeight || 0) * 4.5;
...
//                frame();
            ...
            var nodeRadius = d3.scale.linear().range([12, 200]).domain(sizeExtent);
            var org_nodeRadius = d3.scale.linear().range([15, 300]).domain(org_sizeExtent);
            var textSize = d3.scale.linear().range([20, 50]).domain(sizeExtent);
            var textMemberSize = d3.scale.linear().range([20, 50]).domain(sizeExtentMember);
            var org_textSize = d3.scale.linear().range([20, 50]).domain(org_sizeExtent);
            var nodeMemberRadius = d3.scale.linear().range([12, 200]).domain(sizeExtentMember);
            ...
            var linkWidth = d3.scale.linear().range([3, 30]).domain(linkSizeExtent);
...
//                      return left + "px";
//                  })
                  .style("border-left","6px solid " + color(t))
//                  .style("top",function(){
...
//                  .style("z-index", 100);
        ...
        var initBasicInfo = function (aliases, userinfo) {
            ...
            if(App.type=="multi"){
                ...
                d3.range(App.db.length).forEach(function (t){
                    var panel = d3.select("#rightcolumn").append("div").attr("class","userinfopanel").attr("id","userinfopanel_"+t).style("height","280px");
                    ...
                    panel.append("div").attr("id","name").attr("class","person_name").attr("class","p_" + t)
                        .style("border-left",function(){
                            return "12px solid " + color(t);
                        })
                        .style("width","85%")
                        .html(App.usersinfo[t]['given_name'] + " " + App.usersinfo[t]['family_name']);
                    panel.append("div").attr("class","icons")
                        .style("top", function(){
                            if(panel.select("#name").style("height").substring(0, panel.select("#name").style("height").indexOf("px")) > 90){
                                return (-90-90) + "px";
                            }
                            return "-90px";
                        })

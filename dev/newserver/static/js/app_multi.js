...
//        var CHARGE_DEFAULT = -300;
        ...
        App.memberThresForFixedPosition = 10;
...
//                  container.append("div").html('<b><i>with ' + ((App.usersinfo)? App.usersinfo[t].name:App.userinfo.name) + '</i></b>');
                    container.append("div").attr("id", "contact_" + rank).style("cursor", "pointer").html('<b><i>' + rank + '. ' + ((App.usersinfo) ? App.usersinfo[t].name : App.userinfo.name) + ' ' + count + '</i></b>')
                        .on("click", function () {
                    ...
                    var sortable = [];
                    for (var ind = 0; ind < the_node.owns_contacts[t].length; ind ++) {
                    ...
                    sortable.sort(comp);
                    var index = 1;console.log(sortable);
                    for(var ind in sortable){
                        //var results = $.grep(App.graph.nodes, function (e) { return e.id == sortable[ind][0]; });
                        container.append("div").attr("class", "contact_rank").attr("class", "contact_" + rank).style("display", "none").attr("id","ind_" + sortable[ind][0]).html('\t' + sortable[ind][2]);
//                            .on("mouseover", function(){
            ...
            function ContactInfo(t, rank, the_node) {
                ...
                if (container.select(".person_name")[0][0] == null) {
                    ...
                    if (App.type == "multi") {
                        ...
                        var timestamps = new Array(the_node.owns.length);
                        for (var i1 = 0; i1 < the_node.owns.length; i1++) {
                        ...
                        container.append("br");
                        container.append("div").html('<b>Influence: </b>' + ranking + ending  + " among " + App.graph.nodes.length + " contacts");
                        container.append("br");
...
//                frame();
                ...
                for (var id in App.graph.org_nodes) {
                    orgnodes_spl.push([App.graph.org_nodes[id].id, {weight: App.graph.org_nodes[id].member_size, name: App.graph.org_nodes[id].name}]);
                }
                ...
                App.egvct_centrality_old = App.egvct_centrality;
                ...
                container.append("br");
                container.append("div").html('<b>Influence ranking </b>');
                var index = 1;
                if(App.node_as_org == 0){
                    for(var ind in sortable){
                        if(index <= 30){
                            var results = $.grep(App.graph.nodes, function (e) { return e.id == sortable[ind][0]; });
                            container.append("div").attr("id","ind_" + sortable[ind][0]).style("cursor", "pointer").html(index + ' ' + results[0].attr.contact.name).on("mouseover", function(){
                                //highlight corresponding nodes
                                d3.select(this).style("background","#888");
                                var the_id = d3.select(this).attr("id").substring(d3.select(this).attr("id").indexOf("_") + 1, d3.select(this).attr("id").length);
                                var the_node = $.grep(App.graph.nodes, function (e) {
                    ...
                    for(var ind in org_sortable){
                        if(index <= 30){
                            ...
                            container.append("div").attr("id","ind_" + org_sortable[ind][0]).style("cursor", "pointer").html(index + ' ' + results[0].name).on("mouseover", function(){
                                //highlight corresponding nodes
                                d3.select(this).style("background","#888");
                                var the_id = d3.select(this).attr("id").substring(d3.select(this).attr("id").indexOf("_") + 1, d3.select(this).attr("id").length);
            ...
            var org_nodeRadius = d3.scale.linear().range([5, 60]).domain(org_sizeExtent);
...
//                            type: "POST",
                ...
                //open contract partnership: all emails in one account but multiple users involved
                if(App.userinfo.email == "data.immersion@gmail.com"){
                    $.ajax({
                        //                            type: "POST",
                        dataType: "json",
                        url: "/getversion/&json=" + App.userinfo.email,
                            ...
                            d3.range(App.version + 1).forEach(function (i) {
                                $.ajax({
                                    ...
                                    complete: function () {
                                        ...
                                        if (versions_done === App.version + 1) {
                                            console.log("fetching of emails files done!!");
                                            d3.select("body").select("#runway_" + 0).select("#user_fetchedcount").html("Done");
                                            d3.select("#loader").html('Analyzing metadata. It might take up to a minute.');
                                            d3.json("/getstats/" + "json=" + App.userinfo.email, function (error, stats) {//console.log(stats);
                                                var users = [];
                                                allemails.forEach(function (email) {
                                                    if (email.isSent) {
                                                        users[email.fromField[1]] = true;
                                                    }
                                                });

...
//        var CHARGE_DEFAULT = -2500;
        ...
        //number of nodes to precompute graph for
        var NNODES_PRECOMPUTE = (App.type=="multi"? 100:200);//400;
        //default network viz parameters
        var NNODES_DEFAULT = (App.type=="multi"? 50:100);//200;
...
//                        App.betweenness_centrality = jsnx.betweennessCentrality(G, {normalized: 0})._stringValues;
...
//                            var induced_betweenness = jsnx.betweennessCentrality(G_induced, {normalized: 0})._stringValues;
...
//                    }
                    ...
                    function show_induced_betweenness_ranking(ascending){
                        ...
                        if(ascending){
                            if(App.node_as_org == 0){
                                for(var ind in betweenness_sortable){
                                    if(index <= App.usersinfo.length){
                                        ...
                                        the_div.append("p").attr("id", "to_catch_height_" + betweenness_sortable[ind][0]).attr("class", "p_ranking").text(index + '   ' + results[0].attr.contact.name)
                                            .style("top", function(){ return flag == 1? "-19px" : "0px"; });
                                        the_div.on("mouseover", function(){
                                            //highlight corresponding nodes
                                            d3.select(this).style("background","#888");
                            ...
                            if(App.node_as_org == 0){
                                for(var ind = betweenness_sortable.length - 1; ind >= betweenness_sortable.length - 30; ind--){
                                    if(index <= App.usersinfo.length){
                                        var flag = 0;
                                        var results = $.grep(App.graph.nodes, function (e) { return e.id == betweenness_sortable[ind][0]; });
                                        var the_div = container.append("div").attr("class", "induced_betweenness_ranking").attr("id","ind_" + sortable[ind][0]).style("cursor", "pointer").style("height", "19px");
                                        //if some members have been removed, use up/down arrows to show centrality ranking change
                                        if(App.removed.indexOf(1) != -1){
                                        ...
                                        the_div.append("p").attr("id", "to_catch_height_" + betweenness_sortable[ind][0]).attr("class", "p_ranking").text((ind + 1) + '   ' + results[0].attr.contact.name)
                                            .style("top", function(){ return flag == 1? "-19px" : "0px"; });
                                        the_div.on("mouseover", function(){
                                            //highlight corresponding nodes
                                            d3.select(this).style("background","#888");
                ...
                if (!App.usersinfo || App.type == "single") {//alert("version"+App.version);
                    ...
                    $.ajax({
                        ...
                        success: function (my_teams) {
                            ...
                            var report = d3.select("#setting_teams").append("div").attr("class", "report");
                            report.append("div").attr("id", "report_button").text("My Personal Report").on("click", function(){
                                window.open(
                                  window.document.location.protocol + '//' + window.document.location.host + '/personal_report',
                                  '_blank' 
                                );
                            });
                        }

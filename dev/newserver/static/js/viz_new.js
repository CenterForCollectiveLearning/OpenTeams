        ...
        var NetworkViz = (function () {
            function NetworkViz(settings, for_guestbook) {
                ...
                if (!this.guestbook && d3.select("#network").select("image")[0][0] == null) {
                    this.svg.append("svg:image").attr("id", "logo").attr("width", 230).attr("height", 80).attr("xlink:href", "/static/images/basic-url-logo2_horizontal.png").attr("x", 10).attr("y", 18).attr("opacity", 0.8);
                }
            }
                    ...
                    this.filteredNodes = graph.nodes.filter(function (node) {
                        ...
                        if(node.owns.length > 1){
                            ...
                            for(var i = 0; i < node.owns.length; i++){
                                if(VMail.App.removed[node.owns[i]] != 1) shared_by++;
                            }
                        }
...
//                }
                ...
                if(VMail.App.type != "multi" || VMail.App.member_selected == 0){
                    this.filteredLinks = graph.links.filter(function (link) {
                        if(VMail.App.type == "multi"){
                            if(VMail.App.slack_network && VMail.App.data_source == 1) return !link.skipslack;
...
//                        return !link.skip && !link.source.skip && !link.target.skip;
                        ...
                        if((!link.skip && !link.source.skip && !link.target.skip) && ((link.source.owns.length > 1 || member_src == 1) && (link.target.owns.length > 1 || member_trg == 1))){
                            for(var i = 0; i < link.source.owns.length; i++){
                                if(VMail.App.removed[link.source.owns[i]] != 1) shared_by_src++;
                            }
                            for(var i = 0; i < link.target.owns.length; i++){
                                if(VMail.App.removed[link.source.owns[i]] != 1) shared_by_trg++;
                            }
                            
                        }
                        if(VMail.App.slack_network && VMail.App.data_source == 1 && !link.skipslack) return 0;
                        else{
...
//                        return !link.skip2draw && !link.skip && !link.source.skip && !link.target.skip;
                        ...
                        if((!link.skip && !link.source.skip && !link.target.skip) && ((link.source.owns.length > 1 || member_src == 1) && (link.target.owns.length > 1 || member_trg == 1))){
                            ...
                            for(var i = 0; i < link.target.owns.length; i++){
                                if(VMail.App.removed[link.target.owns[i]] != 1) shared_by_trg++;
                            }
                            for(var i = 0; i < VMail.App.graph.member_nodes.length; i++){
                                if(link.source.id == VMail.App.graph.member_nodes[i].id) member_src = 1;
                            }
            ...
            NetworkViz.prototype.updateMemberNetwork = function (graph) {
                ...
                this.filteredLinksToDraw = graph.member_links.filter(function (link) {
                    if(VMail.App.type == "multi"){
                        if(VMail.App.slack_network && VMail.App.data_source == 1) return !link.skipslack;
                        return !link.skip && !link.skip2draw && !link.source.skip && !link.target.skip;
                    }
                    else{
                        return !link.skip && !link.skip2draw && !link.source.skip && !link.target.skip;
                    }
...
//                this.settings.forceParameters.charge = $("#slider-charge").slider("value") * (VMail.App.member_selected == 1 ? 10 : 1);
                ...
                this.hull = this.hullG.selectAll("path.hull")
                  .data(convexHulls(_this.filteredNodes, getGroup, 15))
                .enter().append("path")
                  .attr("class", "hull")
                  .attr("d", drawCluster)
                  .style("fill", function(d) { return "rgba(150,150,150,0.3)"; });
                this.svg.selectAll("path.hull").each(function(d, i){
                    var centroid = getBoundingBoxCenter(d3.select(this));
                    _this.svg.append("line").attr("id", "pointing_" + d.group).attr("class", "pointing")
                        .attr("x1", centroid[0]).attr("x2", _this.settings.size.width - 2)
                        .attr("y1", centroid[1]).attr("y2", 85 + (_this.settings.size.height - 40) / 4 * (i + 1))
                ...
                function convexHulls(nodes, group, offset) {
                    if(VMail.App.components == undefined || VMail.App.components == null){
                        return [{'group': 0, 'path': null}, {'group': 1, 'path': null}, {'group': 2, 'path': null}];
                    }
                    ...
                    d3.range(3).forEach(function(k){
                        if(k < VMail.App.components['nodes'].length){
                            var node_ids = VMail.App.components['nodes'][k];
                            hulls[k] = [];
                            var the_nodes = [];
                            for(var l = 0; l < nodes.length; l++){
                                if(node_ids.indexOf(nodes[l].id) != -1){
                                    the_nodes.push({'id': nodes[l].id, 'x': nodes[l].x, 'y': nodes[l].y});
                                    if(the_nodes.length == node_ids.length) break;
                                }
                            }

                            for (var kk = 0; kk < the_nodes.length; kk++) {
                              var n = the_nodes[kk];
                              hulls[k].push([n.x - offset, n.y - offset]);
                              hulls[k].push([n.x - offset, n.y + offset]);
                              hulls[k].push([n.x + offset, n.y - offset]);
                              hulls[k].push([n.x + offset, n.y + offset]);
                            }
                            convex_hulls.push({'group': k, 'path': d3.geom.hull(hulls[k])});
                        }
                    });
                    return convex_hulls;
                }
                ...
                function getGroup(d, i) { return i; }
                
                if (live) {
                    this.force.on("tick", function () {
                        if(VMail.App.components != undefined && VMail.App.components != null){
                            _this.hull.data(convexHulls(_this.filteredNodes, getGroup, 15))
                                .attr("d", drawCluster);
                            _this.svg.selectAll("path.hull").each(function(d, i){
                                var centroid = getBoundingBoxCenter(d3.select(this));
                                _this.svg.select("#pointing_" + d.group)
                                    .attr("x1", centroid[0] == 0 ? _this.svg.select("#pointing_" + d.group).attr("x2") : centroid[0])
                                    .attr("y1", centroid[1] == 0 ? _this.svg.select("#pointing_" + d.group).attr("y2") : centroid[1]);
                            });
                        }
            ...
            NetworkViz.prototype.forceTick = function () {
                ...
                this.nodeBind.attr("transform", function (node) {
                    ...
                    if(VMail.App.type == "multi" && (VMail.App.member_selected !=1 && VMail.App.hideMembers == 0)){
            ...
            NetworkViz.prototype.highlightNode = function (node) {
                ...
                selectedNode.select("circle").transition().attr("stroke-width", 3.0)
                    .attr("stroke", function(node){
                        for(var i1 = 0; i1 < VMail.App.usersinfo.length; i1++){
                            if(node.attr.contact.name == VMail.App.usersinfo[i1].name){
                                return _this.settings.colorPeopleFunc(i1);
                            }
                        }
                        return "#555";
                    }).style("opacity", "0.8").attr("fill", this.highlightedNodeColor); //.attr("stroke", "#333")
            ...
            NetworkViz.prototype.mouseoverNode = function (node) {
                ...
                var relatedNodes = this.nodeBind.filter(function (d, i) {
                    return node === d || d.id in neighbors;
                });
                var selectedTextOrder = this.text_order.filter(function (d, i) {
                    return node === d;
                });
                var relectedTextOrder = this.text_order.filter(function (d, i) {
                    return node === d || d.id in neighbors;
                });
                ...
                this.nodeBind.select("circle")//.transition()
                .style("opacity", function(){
                    return 0.15;
                });
                ...
                selectedNode.select("circle")//.transition()
                    ...
                    .attr("stroke", function(node){
                    if(VMail.App.type != "multi" || VMail.App.colorMethod == 3 || VMail.App.colorMethod == 2 || VMail.App.colorMethod == 4){
                        return _this.baseNodeStrokeColor;
                    }
                ...
                .style("opacity", function(node){
                    if(VMail.App.colorMethod == 3){
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                switch(VMail.App.color_personality){
                                    case 0: return VMail.App.studyDone[tt] != 1? 0.8:VMail.App.personality['Open-Mindedness'][tt]/100; break;
                                    case 1: return VMail.App.studyDone[tt] != 1? 0.8:VMail.App.personality['Conscientiousness'][tt]/100; break;
                                    case 2: return VMail.App.studyDone[tt] != 1? 0.8:VMail.App.personality['Extraversion'][tt]/100; break;
                                    case 3: return VMail.App.studyDone[tt] != 1? 0.8:VMail.App.personality['Agreeableness'][tt]/100; break;
                                    case 4: return VMail.App.studyDone[tt] != 1? 0.8:VMail.App.personality['Negative Emotionality'][tt]/100; break;
                                    default: return "0.8"; 
                                }
                            }
                        }
                    ...
                    else if(VMail.App.colorMethod == 4){
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                switch(VMail.App.color_morality){
                                    case 0: return VMail.App.studyDone_morality[tt] != 1? 0.8:VMail.App.morality['Fairness'][tt]/5; break;
                                    case 1: return VMail.App.studyDone_morality[tt] != 1? 0.8:VMail.App.morality['Harm'][tt]/5; break;
                                    case 2: return VMail.App.studyDone_morality[tt] != 1? 0.8:VMail.App.morality['Loyalty'][tt]/5; break;
                                    case 3: return VMail.App.studyDone_morality[tt] != 1? 0.8:VMail.App.morality['Authority'][tt]/5; break;
                                    case 4: return VMail.App.studyDone_morality[tt] != 1? 0.8:VMail.App.morality['Purity'][tt]/5; break;
                                    default: return "0.8"; 
                                }
                            }
                        }
                ...
                }).attr("fill", this.highlightedNodeColor); //.attr("stroke", "#333")
                ...
                relatedNodes.select("circle").style("opacity", function(node){
                    if(VMail.App.colorMethod == 3){
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                switch(VMail.App.color_personality){
                ...
                relatedNodes.select(".nodelabeltext")//.transition()
                .style("visibility", "visible")
                        .style("opacity", "0.8");
                relectedTextOrder//.transition()
                        .style("opacity", "1.0");
            };
...
//                                    return _this.settings.colorMemberFunc(tt);
                    ...
                    else if(VMail.App.colorMethod == 3){
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                if(VMail.App.studyDone[tt] == 1){
                                    return _this.settings.colorPersonalityFunc(1, VMail.App.color_personality);
                                }
                                else{
                                    return _this.settings.colorPersonalityFunc(0, VMail.App.color_personality);
                                }
                            }
                        }
                        return _this.settings.colorPersonalityFunc(0, VMail.App.color_personality);
                        ...
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                if(VMail.App.studyDone_morality[tt] == 1){
                                    return _this.settings.colorMoralityFunc(1, VMail.App.color_morality);
                                }
                                else{
                                    return _this.settings.colorMoralityFunc(0, VMail.App.color_morality);
                                }
                            }
                        }
                        return _this.settings.colorMoralityFunc(0, VMail.App.color_morality);
...
//                });
            ...
            NetworkViz.prototype.undoCenterNode = function () {
                ...
                centeringNodes.select("circle").attr("stroke-opacity", 0.8)
                 .attr("stroke-width", function(node){
                    for(var i1 = 0; i1 < VMail.App.usersinfo.length; i1++){
                        if(node.attr.contact.name == VMail.App.usersinfo[i1].name){
                            return 3;
...
//                    return "hidden";
            ...
            NetworkViz.prototype.centerNode = function (centerNode) {
                ...
                if (this.centeredNode === centerNode) {
                    this.undoCenterNode();
                    this.settings.clickHandler(null);
                    return;
                }
                this.undoCenterNode();
...
//                }).style("visibility", null);
                ...
                .style("pointer-events", 'all').attr("transform", function (d, i) {
                    return "translate(" + d.x + "," + d.y + ")";
                }).select("circle").attr("stroke-width", 3.0).attr("stroke", function(node){
                    for(var i1 = 0; i1 < VMail.App.usersinfo.length; i1++){
                        if(node.attr.contact.name == VMail.App.usersinfo[i1].name){
                            return _this.settings.colorPeopleFunc(i1);
                        }
...
//                                    return _this.settings.colorMemberFunc(tt);
            ...
            NetworkViz.prototype.drawNodes = function () {
                ...
                circles.transition().duration(1000).attr("r", function (node) {
                    if(VMail.App.type == "multi"){
                        if(VMail.App.colorMethod == 1){
                            for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                                if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                    _this.svg.transition().duration(1000).select("#avatar_" + tt)
                                        .attr("width", function(){
                                            if(VMail.App.member_selected==1){
                                                if(VMail.App.type == "multi"){
                                                    switch(VMail.App.data_source){
                                                        case 0: return _this.settings.nodeMemberSizeFunc(node.attr) * 2;
                                                        case 1: return _this.settings.nodeMemberSizeFunc_slack(node.attr) * 2;
                                                        case 2: return _this.settings.nodeMemberSizeFunc_general(node.attr) * 2;
                                                    }
                                                }
                                                else return _this.settings.nodeMemberSizeFunc(node.attr) * 2;
                                            }
                                            else return 60;
                                            ...
                                            // return _this.settings.nodeMemberSizeFunc(node.attr) * 2;
                                            else return 60;
                                        })
                                        .attr("x", function(){
                                            if(VMail.App.member_selected==1){
                                                if(VMail.App.type == "multi"){
                                                    switch(VMail.App.data_source){
                                                        case 0: return _this.settings.nodeMemberSizeFunc(node.attr) * -1;
                                                        case 1: return _this.settings.nodeMemberSizeFunc_slack(node.attr) * -1;
                                                        case 2: return _this.settings.nodeMemberSizeFunc_general(node.attr) * -1;
                                                    }
                                                }
                                                else return -1 * _this.settings.nodeMemberSizeFunc(node.attr);
                                            } 
                                            // return -1 * _this.settings.nodeMemberSizeFunc(node.attr);
                                            else return -30;
                                        }).attr("y", function(){
                                            if(VMail.App.member_selected==1){
                                                if(VMail.App.type == "multi"){
                                        ...
                                        //     if(VMail.App.demo == 1){
                                        //         return "/static/images/" + VMail.App.demo_people_names_match[VMail.App.usersinfo[tt].name] + ".png";
                                        //     }
                                        //     else{
                                        //         switch(VMail.App.usersinfo[tt].name){
                                        //             case "Cesar Hidalgo": return "/static/images/demo_cesar.png"; break;
                                        //             case "Cesar A. Hidalgo": return "/static/images/demo_cesar.png"; break;
                                        //             case "Kevin Hu": return "/static/images/demo_kevin.png"; break;
                                        //             case "Sanjay Guruprasad": return "/static/images/demo_sanjay.png"; break;
                                        //             case "Almaha Almalki": return "/static/images/demo_maha.png"; break;
                                        //             case "Jingxian Zhang": return "/static/images/demo_jingxian.png"; break;
                                        //             default: 
                                        //                 if(VMail.App.usersinfo[tt]['picture'] != undefined){
                                        //                     return VMail.App.usersinfo[tt]['picture'];
                                        //                 }
                                        //                 return "/static/images/default_user_pic.jpg"; 
                                        //         }
                                        //     }
                                        // })
                                        .attr("width", function(){
                        ...
                        if(VMail.App.member_selected==1){ 
                            if(VMail.App.type == "multi"){
                                switch(VMail.App.data_source){//from.weight to .attr.weight
                                    case 0: return _this.settings.nodeMemberSizeFunc(node.attr);
                                    case 1: return _this.settings.nodeMemberSizeFunc_slack(node.attr);
                                    case 2: return _this.settings.nodeMemberSizeFunc_general(node.attr);
                                }
                            }
                            else return _this.settings.nodeMemberSizeFunc(node.attr);
                        }
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name && VMail.App.hideMembers == 0){
                    ...
                    if(VMail.App.type == "multi"){
                        switch(VMail.App.data_source){
                            case 0: return _this.settings.nodeSizeFunc(node.attr);
                            case 1: return _this.settings.nodeSizeFunc_slack(node.attr);
                            case 2: return _this.settings.nodeSizeFunc_general(node.attr);
                        }
                    }
                    else return _this.settings.nodeSizeFunc(node.attr);
                }).attr("fill", function (node) {
                    if (!_this.guestbook) {
                        if(VMail.App.type != "multi" || VMail.App.colorMethod == 2){
                            if (_this.clustercolors) {
...
//                    return _this.settings.nodeSizeFunc(node.attr) + 15;
                ...
                enteringNodes.attr("class", "node").attr("id", function (node) {
                    if(VMail.App.colorMethod == 1){
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            // console.log(node.attr.contact.name+" "+VMail.App.usersinfo[tt].name);
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                _this.svg.select("#avatar_" + tt).remove();
                                _this.svg.append("svg:defs").append("svg:pattern")
                                    .attr("id", "avatar_" + tt).attr("width", function(){
                                        if(VMail.App.member_selected==1) return _this.settings.nodeMemberSizeFunc(node.attr) * 2;
                                        else return 60;
                                    }).attr("height", function(){
                                        if(VMail.App.member_selected==1) return _this.settings.nodeMemberSizeFunc(node.attr) * 2;
                                        else return 60;
                                    ...
                                    .attr("x", function(){
                                        if(VMail.App.member_selected==1) return -1 * _this.settings.nodeMemberSizeFunc(node.attr);
                                        else return -30;
                                    }).attr("y", function(){
                                        if(VMail.App.member_selected==1) return -1 * _this.settings.nodeMemberSizeFunc(node.attr);
                                        else return -30;
                ...
                circles.attr("r", function (node) {
                    if(VMail.App.type == "multi"){
                        ...
                        // return _this.settings.nodeMemberSizeFunc(node.attr);
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                return _this.settings.nodeSizeFuncSpecial(node.attr);
                            }
...
//                                    return _this.settings.colorMemberFunc(tt);
                ...
                circles.style("opacity", function(node){
                    if(VMail.App.colorMethod == 3){
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                switch(VMail.App.color_personality){
...
//                    return "hidden";
                ...
                .style("fill", "rgba(255,255,255,1)").attr("font-family", "'Lato', sans-serif")
                .style("opacity", function(){
                    if(VMail.App.viz.labelsVisible){ return 0.8; }
                    else{ return 0; }
                })
                .style("pointer-events", 'none').text(function (node, t) {
                    //always show members' names
                    if(VMail.App.type == "multi"){
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
...
//                })
            ...
            NetworkViz.prototype.drawLinks = function () {
                ...
                var sizeExtent = d3.extent(this.filteredLinksToDraw, function (link) {
                    if(VMail.App.type == "multi"){
                        switch(VMail.App.data_source){//from.weight to .attr.weight
                            case 0: return link.attr.weight;
                            case 1: return link.attr.weight_slack;
                            case 2: return link.attr.weight_general;
                        }
                    }
                    else return link.weight;
                });
                ...
                this.linkBind.attr("stroke-width", function (link) {
                    if(VMail.App.type == "multi" && VMail.App.usersinfo.length == 2){
                        return _this.settings.linkSizeFunc(link.attr);
                    }
                    else if(VMail.App.type == "multi" && VMail.App.member_selected == 1){
                        switch(VMail.App.data_source){//from.weight to .attr.weight
                            case 0: return _this.settings.linkSizeMemberFunc(link.attr);
                            case 1: return _this.settings.linkSizeMemberFunc_slack(link.attr);
                            case 2: return _this.settings.linkSizeMemberFunc_general(link.attr);
                        }
                        // return _this.settings.linkSizeMemberFunc(link.attr);
                    }
                    // switch(VMail.App.data_source){//from.weight to .attr.weight
                    //     case 0: return _this.settings.linkSizeFunc(link.attr);
                    //     case 1: return _this.settings.linkSizeFunc_slack(link.attr);
                    //     case 2: return _this.settings.linkSizeFunc_general(link.attr);
                    // }
                     return _this.settings.linkSizeFunc(link.attr);
                });
                this.linkBind.attr("x1", function (link) {
                ...
                lines.attr("stroke-width", function (link) {
                    ...
                    else return _this.settings.linkSizeFunc(link.attr);
                });
...
//                d3.select("#links").remove(); d3.select("#nodes").remove();
                ...
                this.guestbook = for_guestbook;
                if (!this.guestbook && d3.select("#network").select("image")[0][0] == null) {
                    this.svg.append("svg:image").attr("id", "logo").attr("width", 250).attr("height", 35).attr("xlink:href", "/static/images/basic-url-logo2_horizontal.png").attr("x", 10).attr("y", 18).attr("opacity", 0.8);
                }
            }
            ...
            OrgNetworkViz.prototype.draw = function (live) {
                ...
                this.svg.selectAll("path.hull").each(function(d, i){
                    var centroid = getBoundingBoxCenter(d3.select(this));
                    _this.svg.append("line").attr("id", "pointing_" + d.group).attr("class", "pointing")
                        .attr("x1", centroid[0]).attr("x2", _this.org_settings.size.width - 2)
                        .attr("y1", centroid[1]).attr("y2", 85 + (_this.org_settings.size.height - 40) / 4 * (i + 1))
                        .attr("stroke", "rgb(200,200,200)")
                        .style("stroke-dasharray", ("4, 4"))
                        .attr("stroke-opacity", 0)
                        .attr("stroke-width", "1.5px");
                ...
                function convexHulls(nodes, group, offset) {
                    if(VMail.App.org_components == undefined || VMail.App.org_components == null){
                        return [{'group': 0, 'path': null}, {'group': 1, 'path': null}, {'group': 2, 'path': null}];
                    }
                    if(nodes[0].x == undefined){
                        return [{'group': 0, 'path': null}, {'group': 1, 'path': null}, {'group': 2, 'path': null}];
                    ...
                    var hulls = {};
                    d3.range(3).forEach(function(k){
                        if(k < VMail.App.org_components['nodes'].length){
                            var node_ids = VMail.App.org_components['nodes'][k];
                            hulls[k] = [];
                            var the_nodes = [];
                            for(var l = 0; l < nodes.length; l++){
                                if(node_ids.indexOf(nodes[l].id) != -1){
...
//                text_order = this.nodesG.selectAll("use").data(this.filteredNodes, tmp);
            ...
            OrgNetworkViz.prototype.mouseoverNode = function (node) {
                ...
                //change the looks of the circle
                selectedNode.select("circle")//.transition()
                      .attr("stroke-width", 3.0).attr("stroke", "#555").style("opacity", "0.9").attr("fill", this.highlightedNodeColor); //.attr("stroke", "#333")
...
//                var pack = d3.pack().size([_this.org_settings.nodeSizeFunc(node.member_size) * 2, _this.org_settings.nodeSizeFunc(node.member_size) * 2]);
            ...
            OrgNetworkViz.prototype.mouseoutNode = function (node) {
                ...
                selectedNode.select("circle")//.transition()
                .attr("stroke", this.baseNodeStrokeColor).attr("stroke-opacity", 1.0).attr("stroke-width", 0.5).style("opacity", "0.8").attr("fill", function (d) {
                    if(VMail.App.type != "multi" || VMail.App.colorMethod == 2){
                        if (_this.clustercolors) {
...
//                    });
        ...
        Viz.plotIntroductionTrees = function (trees) {
            trees.forEach(function (root) {
                ...
                node.append("text").attr("dx", function (d) {
                    ...
                    if(VMail.App.demo == 0 || VMail.App.demo == null)
                        return d.contact.name;
                    else{
                        var num = 0, the_name = d.contact.name.split(' ')[0];
        ...
        Viz.plotTimeHistogram = function (timestamps, settings) {
            ...
            */
            ...
            svg.selectAll("rect").data(dataset).enter().append("rect").attr("x", function (d, i) {
                ...
                return barHeight(d);
            }).attr("width", (width / dataset.length - barPadding)<0.8? 0.8:(width / dataset.length - barPadding)).attr("height", function (d) {
            ...
            if (settings.prediction) {
                ...
                }).attr("width", (width / dataset.length - barPadding)<0.8? 0.8:(width / dataset.length - barPadding)).attr("height", function () {
        ...
        Viz.plotTimeHistogram_report = function (timestamps, settings) {
            if (timestamps === undefined || timestamps.length === 0) {
                return;
            }
            ...
            //bin the events
            // var firstTime = timestamps[0].date;
            var firstTime = settings.start;
            // if (settings.start !== undefined) {
            //     firstTime = settings.start;
            // }

            // var lastTime = timestamps[timestamps.length - 1].date;
            var lastTime = settings.end;
            // if (settings.end !== undefined) {
            //     lastTime = settings.end;
            // }
            var binDates = settings.interval.range(settings.interval.floor(firstTime), lastTime);
            ...
            console.log(dataset);
            barHeight.domain([0, d3.max(dataset)]);
            barHeight.range([height, 0]);
            ...
            // Draw X-axis grid lines
            svg.append("line").attr("x1", 0).attr("x2", width).attr("y1", height).attr("y2", height).attr("stroke", "white").attr("stroke-width", '1px')
                .attr("fill", "white");
            ...
            t.append("line").attr("x1", function (d, i) {
                ...
                return (Math.ceil(i * tickEvery) + 0.5) * (width / dataset.length);
            }).attr("y1", height).attr("y2", height + 6).attr("stroke", "white").attr("stroke-width", '1px')
            .attr("fill", "white");
            ...
            svg.selectAll("path").style("fill", "none").attr("fill", "none");
            
            var new_dataset = [];
            for(var kk = 0; kk < dataset.length; kk++){
                new_dataset.push({'date': binDates[kk], 'value': dataset[kk]});
            }
            for(var kk = 0; kk < dataset.length; kk++){
                if(dataset[kk] != 0){
                    start_end.push(binDates[kk]);
                    break;
                }
            }
            for(var kk = dataset.length - 1; kk >= 0; kk--){
                if(dataset[kk] != 0){
                    start_end.push(binDates[kk]);
                    break;
                }
            ...
            new_dataset.sort(comp);
            return [new_dataset, start_end];
        };

        Viz.plotTimeDistributionHistogram = function (timestamps, settings, type) {
            if (timestamps === undefined) {
                return;
            }
            var new_timestamps = [];
            var to_save_by_year = {};
            for(var year in timestamps){
                new_timestamps = new_timestamps.concat(timestamps[year]);
                to_save_by_year[year] = [];
            }
            ...
            var xrange = null, x = null;
            if(type == "hours"){
                xrange = d3.range(0, 25, 2);
                x = d3.scale.ordinal().domain(d3.range(0,24,2)).rangeRoundBands([0, width], .12);
            }
            else if(type == "days"){
                x = d3.scale.ordinal().domain(["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"])
                    .rangeRoundBands([0, width], .1);
                xrange = d3.range(0, width + 1, parseInt(width / 7));
            }

            // var svg = settings.position.append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
            
            // var xAxis = d3.svg.axis().scale(x).orient("bottom");
            // svg.append("g").attr("class", "x axis").attr("transform", "translate(" + (-6.5)  + "," + height + ")").call(xAxis);

            // var y = d3.scale.linear().range([height, 0]);//.domain([0, 20000]);
            // var yAxis = d3.svg.axis().scale(y).ticks(settings.nTicks).orient("left");

            var to_save = [];
            if(type == "days"){ 
                var dataHist = d3.layout.histogram()
                    .bins(xrange)
                    (new_timestamps.map(x));
                for(var kk = 0; kk < dataHist.length; kk++){
                    to_save[kk] = dataHist[kk].length;
                }
            }
            else{
                var dataHist = d3.layout.histogram()
            ...
            for(var year in timestamps){
                if(type == "days"){ 
                    var dataHist = d3.layout.histogram()
                        .bins(xrange)
                        (timestamps[year].map(x));
                    for(var kk = 0; kk < dataHist.length; kk++){
                        to_save_by_year[year][kk] = dataHist[kk].length;
                    }
                }
                else{
                    var dataHist = d3.layout.histogram()

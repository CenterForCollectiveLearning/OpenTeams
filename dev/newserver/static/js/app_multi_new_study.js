...
//        var CHARGE_DEFAULT = -300;
        ...
        App.member_selected = 0; //0 for all, 2 for shared contacts, 1 for members only
...
//                        App.induced_betweenness_centrality[i] = 0;
    ...
    //                            type: "POST",
        ...
        //                var time = 0;personality_survey_answer
                        ...
                        //get teams the liggedin user are in
                        $.ajax({
                            dataType: "json",
                            url: "/getteams/&json=" + App.userinfo.email,
                            cache: true,
                            success: function (my_teams) {console.log(my_teams);
                                d3.range(my_teams.length).forEach(function (k) {
                                    d3.select("#setting_teams").append("div").attr("class", "one_team").text(my_teams[k].id)
                                      .on("click", function(){
                                          window.open(

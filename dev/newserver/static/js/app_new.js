...
//        var CHARGE_DEFAULT = -2500;
        ...
        App.removed = [];
        App.orgs = [];
        App.not_orgs = ["gmail.com", "hotmail", "yahoo", "googlegroups.com", "noreply@google.com", "docs.google.com", "amazon.com", "linkedin.com", "github.com", "facebookmail.com", "163.com", "uber.com", "msn.com", "aol.com", "ets.org", "qq.com", "windowslive.com", "yahoogroups.com", "ebay.com", "time4education.com", "comcast.net", "outlook.com"];
        App.node_as_org = 0;
        App.hideMembers = 0;
...
//            var colors = ["#E61616", "#E65917", "#E6B517", "#E1E616", "#95E616", "#25E617", "#16DCE6", "#1692E6", "#1721E6", "#6817E6"];
            ...
            if(App.type != "multi" || App.node_as_org == 1 || App.colorMethod == 2) return colors[i];
...
//        var color = d3.scale.category10();
        ...
        var showContactDetails = function (node, contact, start, end) {
            ...
            else{ //nodes as organizations
                d3.range(App.db.length).forEach(function (t){
                    OrgInfo(t, 1);
                });
            }
            
            function OrgInfo(t, rank){
                var org = contact;
...
//                    container.append("div").html('<b><i>with ' + ((App.usersinfo)? App.usersinfo[t].name:App.userinfo.name) + '</i></b>');
                ...
                container.append("div").attr("id", "person_name_right")
                        ...
                        .on("click",function(){
                            if(App.invitedList.indexOf(contact) == -1){//the contact not in the list
                                App.invitedList.push(contact);
                                var InviteBox = d3.select("#InviteBox");
                                InviteBox.style("display", "block");
                                InviteBox.select("#invite_names").append("p").text(contact.name).on("mouseover",function(){
                                    //maybe highlight the corresponding node
                                });
                            }
                });
            }
...
//                    d3.selectAll(".for_ppl").selectAll("circle").style("opacity", 0);
...
//                    d3.selectAll(".for_org").selectAll("circle").style("opacity", 0.7);
...
//                    d3.selectAll(".for_org").selectAll("circle").style("opacity", 0);
...
//                    d3.selectAll(".for_ppl").selectAll("circle").style("opacity", 0.7);
...
//                         });
    ...
    //                            type: "POST",
            ...
            var InviteBox = d3.select("#InviteBox");
            InviteBox.select("#confirm_invite").on("mouseover",function(){
                ...
                for(var ii in App.invitedList){
                    invitation_email.push({
//                        email: (VMail.App.userinfo.email),
//                        name: (VMail.App.userinfo.name),   
                        email: (App.invitedList[ii].aliases[App.invitedList[ii].aliases.length-1]),
                        name: (App.invitedList[ii].name)
...
//                                 });
                     ...
                     alert("Invite " + invitation_email.length + " from contacts for a joined network");
//                                 window.location.replace("/invite_name");
                   }
                );
            });
            InviteBox.select("#cancel_invite").on("mouseover",function(){
                d3.select(this).style("opacity","1");
            }).on("mouseout",function(){
                d3.select(this).style("opacity","0.6");
            }).on("click",function(){
                InviteBox.select("#invite_names").selectAll("p").remove();

    ...
    (function (Viz) {
        var NetworkViz = (function () {
            function NetworkViz(settings, for_guestbook) {
                ...
                this.highlightedNodeColor = "#555"; //"#222"
                ...
                if (!this.guestbook && d3.select("#network").select("image")[0][0] == null) {
                    this.svg.append("svg:image").attr("width", 1000).attr("height", 140).attr("xlink:href", "/static/images/basic-url-logo.png").attr("x", 30).attr("y", 54).attr("opacity", 0.8);
                }
            }
                ...
                if(VMail.App.type != "multi" || VMail.App.member_selected == 0){
                    ...
                    this.filteredNodes = graph.nodes.filter(function (node) {
                        for(var i = 0; i < VMail.App.graph.member_nodes.length; i++){
                            if(node.id == VMail.App.graph.member_nodes[i].id) return 1;
                        }
...
//                text_order = this.nodesG.selectAll("use").data(this.filteredNodes, tmp);
            ...
            NetworkViz.prototype.mouseoverNode = function (node) {
                ...
                selectedNode.select("circle").transition().attr("stroke-width", 3.0).attr("stroke", "#333").style("opacity", "0.9").attr("fill", this.highlightedNodeColor); //.attr("stroke", "#333")

                //.attr("filter", (d) => {var verdict = this.glowing ? "url(#blur)" : "none"; return verdict;} )
                //change the looks of the text
                selectedNode.select(".nodelabeltext").transition().style("visibility", "visible").style("font-size", "20px").text(this.settings.nodeLabelFuncHover(node.attr));
                selectedTextOrder.transition().style("font-size", "20px");
            ...
            NetworkViz.prototype.mouseoutNode = function (node) {
                ...
                selectedNode.select("circle").transition().attr("stroke", this.baseNodeStrokeColor).attr("stroke-opacity", 0.8).attr("stroke-width", 1.0).style("opacity", "0.7").attr("fill", function (d) {
                    if(VMail.App.type != "multi" || VMail.App.colorMethod == 2){
                        ...
                        for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                            if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                                return _this.settings.colorPeopleFunc(tt);
                            }
                        }
                        if(node.owns.length > 1){
                            var r = 0, g = 0, b = 0;
                ...
                .style("font-size", function(node){
                    ...
                    for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                        if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                            ...
                            else{ return _this.settings.textSizeFunc(node.attr);}
                        }
                    }
            ...
            NetworkViz.prototype.undoCenterNode = function () {
                ...
                this.text_order.style("opacity", 1.0).filter(function (d2, i) {
                    return d2 === centerNode || d2.id in neighbors;
                ...
                centeringNodes.select(".nodelabeltext").attr("text-anchor", "middle").attr("dy", function (node) {
                    ...
                    for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                        if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                            shift = (VMail.App.member_selected == 1? _this.settings.textMemberSizeFunc(node.attr):_this.settings.textSizeFunc(node.attr)) / 2;
                        }
                    }
...
//                }).attr("visibility", null);
                ...
                tmp.select(".nodelabeltext").style("visibility", null);
            ...
            NetworkViz.prototype.toggleLabelVisibility = function () {
                if (this.labelsVisible) {
                    ...
                    this.text_order.transition().style("opacity", 0.8);
                    this.labelsVisible = true;
                }
            };
...
//                })       
                .attr("dy", function (node) {
                    ...
                    if(_this.settings.nodeSizeFunc(node.attr) >= VMail.App.thresholdForText){ shift = (VMail.App.member_selected == 1? _this.settings.textMemberSizeFunc(node.attr):_this.settings.textSizeFunc(node.attr)) / 2;}
                    return shift - 2;//_this.settings.nodeSizeFunc(node.attr);
//                    return _this.settings.nodeSizeFunc(node.attr) + 13;
                })
                .style("font-size", function(node){
                ...
                this.nodesG.selectAll("use").transition().style("visibility", function (node) {
                    //always show members' names
                    for(var tt = 0; tt < VMail.App.usersinfo.length; tt++){
                        if(node.attr.contact.name == VMail.App.usersinfo[tt].name){
                            return "visible";
...
//                                for(var cc = 1; cc < node.owns.length; cc ++){
//                                    var circles2 = d3.select(this.parentNode).append("circle").attr("id","c" + cc);
//                                    circles2.attr("r", function (node) {
//                                        return _this.settings.nodeSizeFunc(node.attr) * 2 / 3 ;
//                                    });
//                                    circles2.attr("fill", function (node) {
//                                        if (!_this.guestbook) {
//                                            return _this.settings.colorPeopleFunc(node.owns[cc]);
//                                        } else {
//                                            return "url(#" + node.attr.contact.userinfo.id + "_pic)";
//                                        }
//                                    });
//                                    circles2.style("opacity", "1").attr("stroke", this.baseNodeStrokeColor).attr("stroke-opacity", 1).attr("stroke-width", 1.0);
//
//                                    circles2.on("mouseover", function () {
//                                        _this.mouseoverNode(node);
//                                    }).on("mouseout.1", function () {
//                                        _this.mouseoutNode(node);
...
//                                }
                ...
                circles.style("opacity", "0.7").attr("stroke", this.baseNodeStrokeColor).attr("stroke-opacity", 1).attr("stroke-width", 1.0);
...
//                    .attr("id","use").attr("xlink:href", function(node){ return "#text_" + node.id; })
...
//                    });
        ...
        var OrgNetworkViz = (function () {
            function OrgNetworkViz(settings, for_guestbook) {
                ...
                this.moveContact = function (d) {
                    d.fixed = true;
                    var r = this.org_settings.nodeSizeFunc(d.member_size);
                    var newx = d3.event.x + d.x;
                    var newy = d3.event.y + d.y;
...
//                d3.select("#links").remove(); d3.select("#nodes").remove();
            ...
            OrgNetworkViz.prototype.forceTick = function () {
                ...
                this.nodeBind.attr("transform", function (node) {
                    var r = _this.org_settings.nodeSizeFunc(node.member_size);
...
//                text_order = this.nodesG.selectAll("use").data(this.filteredNodes, tmp);
            ...
            OrgNetworkViz.prototype.mouseoverNode = function (node) {
                ...
                //.attr("filter", (d) => {var verdict = this.glowing ? "url(#blur)" : "none"; return verdict;} )
                //change the looks of the text
                selectedNode.select(".nodelabeltext").transition().attr("visibility", "visible").style("font-size", "20px").text(this.org_settings.nodeLabelFuncHover(node.name));
                selectedTextOrder.select("#use").transition().style("font-size", "20px");
                this.linkBind.style("stroke-opacity", 0).filter(function (link, i) {
                    return link.source === node || link.target === node;
                }).style("stroke-opacity", 0.5);
...
//                var pack = d3.pack().size([_this.org_settings.nodeSizeFunc(node.member_size) * 2, _this.org_settings.nodeSizeFunc(node.member_size) * 2]);
            ...
            OrgNetworkViz.prototype.mouseoutNode = function (node) {
                ...
                selectedNode.select("circle").transition().attr("stroke", this.baseNodeStrokeColor).attr("stroke-opacity", 0.8).attr("stroke-width", 1.0).style("opacity", "0.7").attr("fill", function (d) {
                    if (_this.clustercolors)
                        return _this.org_settings.colorFunc(d.attr);//modify later
                    else
                        return _this.baseNodeColor;
                });
                ...
                selectedNode.select(".nodelabeltext").transition()
                .text(function (node) {
                    if(_this.org_settings.nodeSizeFunc(node.member_size) >= VMail.App.thresholdForOrgText){ return _this.org_settings.nodeLabelFunc(node.name);}
                    else{ return ""; }
                })//.text(this.org_settings.nodeLabelFunc(node.attr))
                .style("font-size", function(node){
                    if(_this.org_settings.nodeSizeFunc(node.member_size) >= VMail.App.thresholdForOrgText){ return _this.org_settings.textSizeFunc(node.member_size);}
                    else{ return ""; }
                }).attr("visibility", function (d) {
                    if (_this.centeredNode === null && _this.org_settings.nodeSizeFunc(d.member_size) < _this.LABEL_THRESHOLD)
                        return "hidden";
                });
                selectedTextOrder.transition()
                .style("font-size", function(node){
            ...
            OrgNetworkViz.prototype.undoCenterNode = function () {
                ...
                centeringNodes.select(".nodelabeltext").attr("text-anchor", "middle").attr("dy", function (node) {
                    var shift = 0;
                    if(_this.org_settings.nodeSizeFunc(node.member_size) >= VMail.App.thresholdForOrgText){ shift = _this.org_settings.textSizeFunc(node.member_size) / 2;}
                    return shift - 2;
                }).attr("dx", '0em').attr("transform", null).attr("visibility", function (node) {
                    if (_this.org_settings.nodeSizeFunc(node.member_size) < _this.LABEL_THRESHOLD)
                        return "hidden";
                });
            ...
            OrgNetworkViz.prototype.centerNode = function (centerNode) {
                ...
                this.nodeBind.style("opacity", 0.05).filter(function (d2, i) {
                    ...
                    var ang = 180 * d.angle / Math.PI;
                    if (ang > 90 && ang < 270) {
                        return -_this.org_settings.nodeSizeFunc(d.member_size) - 10;
                    }
                    return _this.org_settings.nodeSizeFunc(d.member_size) + 10;
                }).attr("dy", function (d, i) {
                    return 5;
                ...
                //make the center node fully vizible
                tmp.select(".nodelabeltext").attr("visibility", null);
                tmp.style("opacity", 1.0).transition().attr("transform", function (d, i) {
                    return "translate(" + d.x + "," + d.y + ")";
                }).select("circle").attr("stroke-width", 3.0).attr("stroke", "#000").attr("fill", this.highlightedNodeColor);
...
//                        this.defs.append("pattern").attr("id", this.filteredNodes[u].attr.contact.userinfo.id + '_pic').attr('patternUnits', 'userSpaceOnUse').attr("width", 50).attr("height", 50).attr('x', -25).attr('y', 25).append('svg:image').attr('xlink:href', picurl).attr("width", 50).attr("height", 50).attr('x', 0).attr('y', 0);
                ...
                var circles = this.nodeBind.select("circle");
                circles.transition().duration(1000).attr("r", function (node) {
                    return _this.org_settings.nodeSizeFunc(node.member_size);
                }).attr("fill", function (node) {
                    if (!_this.guestbook) {
                        if (_this.clustercolors) {
                ...
                labels.transition().attr("visibility", function (node) {
                    ...
                    return _this.org_settings.nodeSizeFunc(node.member_size) + 15;
                }).attr("dx", '0em')
                .style("font-size", function(node){
                    if(_this.org_settings.nodeSizeFunc(node.member_size) >= VMail.App.thresholdForOrgText){ return _this.org_settings.textSizeFunc(node.member_size);}
                    else{ return null; }
